    ...
    public void setFromFile(String fileName) {
        try (BufferedReader reader = new BufferedReader(new FileReader(fileName))) {
            ...
            while ((line = reader.readLine()) != null && row < SIZE) {
                ...
                // Expect: "RR CCCCCC RR"
                String content = line.substring(3, 3 + SIZE);
                for (int col = 0; col < SIZE; col++) {
                    char symbol = content.charAt(col);
                    switch (symbol) {
                        case 'N': board[row][col] = new Piece("licorne", "noir");  break;
                        case 'n': board[row][col] = new Piece("paladin", "noir");  break;
                        case 'B': board[row][col] = new Piece("licorne", "blanc"); break;
                        case 'b': board[row][col] = new Piece("paladin", "blanc"); break;
                        case '-': board[row][col] = null; break;
                        default:  throw new RuntimeException("Symbole non reconnu : " + symbol);
                    }
                }
                row++;
            }
            // After loading a position, we must reset turn and last lisere
            currentPlayer = "blanc";
